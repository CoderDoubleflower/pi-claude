...
pi-claude -r                  # Browse and select a session
pi-claude --no-session        # Ephemeral mode; do not save
pi-claude --name "my task"    # Set session display name at startup
pi-claude --session <path|id> # Use a specific session file or session ID
pi-claude --fork <path|id>    # Fork a session into a new session file
```
...
pi-claude update --all              # Update pi and packages; reconcile pinned git refs
pi-claude update --extensions       # Update packages only; reconcile pinned git refs
...
These commands manage pi packages and `pi-claude update` can update the pi CLI installation. To uninstall pi itself, see [Quickstart](quickstart.md#uninstall). `pi-claude config` and project package commands accept `--approve`/`--no-approve` to trust or ignore project-local settings for one command. `pi-claude update` never prompts for project trust.
...
```bash
pi-claude --no-extensions -e ./my-extension.ts
```
