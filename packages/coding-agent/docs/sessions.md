...
pi-claude --session <path|id> # Use a specific session file or partial session ID
pi-claude --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `pi-claude -r` opens the same picker at startup.
...
```bash
pi-claude --name "Refactor auth module"
pi-claude --name "CI audit" -p "Review this build failure"
```

Named sessions are easier to find in `/resume` and `pi-claude -r`.
